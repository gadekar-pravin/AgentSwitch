...
Status: agreed by the team (2026-09-19). Phases 1 to 7 (and 2b) are done (see section 10). Phase 8's
switch criteria are met (2026-09-19); its exit, team sign-off, is pending.
...
| `agentswitch/executor.py` | Runs ready nodes, frontier settling, exclusive and terminal rules, calls the planner; `run_graph_agent()`, the graph agent's entry point |
| `agentswitch/reads.py` | MCP read workers shared by the old loop and the graph: list paging, result projections, the recording proxy, and the pinned guarded reschedule |
...
| 2b | Task data to `tasks.jsonl` (harness only; can run beside 3–6). Done 2026-09-19: `harness/tasks.py` `load_tasks()` validates the file strictly; an unknown selector or `request_kind`, or any malformed line, is a configuration error (exit 2). Run records keep the old eight task fields; `expectation` is not recorded until the judge uses it | `deterministic` verdicts unchanged. Met 2026-09-19 on Suryodaya: 6 pass, `reschedule_own_draft` `not_applicable` without `--allow-draft-writes`, as before |
| 3 | Capability registry from the live catalogue; the current loop uses `validate()`. Done 2026-09-19: `agentswitch/capabilities.py` holds the allowlist, list filter sets, `Capability`, `Manifest`, `build_manifest()` and `validate()`; `reschedule_work_order` and `answer` are registered (`answer` is still offered to the `llm` loop as `finish`, whose checks are unchanged). The loop validates every read and the reschedule shape before calling MCP. A tool whose exposed schema the gate does not support is dropped and listed in the run record's `manifest.dropped`; `format` is treated as an annotation | The read part of the manifest matches today's `build_tool_menu` on the same catalogue. Met 2026-09-19: the whole menu (read tools, the two local functions, order and native names) is identical to `3d42891`'s on the saved Suryodaya and Keystone catalogues, with no drops |
| 4 | `uv add networkx`; graph, journal, executor (`max_workers = 1`), planner with `plan_frontier`, critic and repairs, terminal worker, audit checks from 7.4 and their `_score_verdict` wiring, offline transports; `graph` subject on read-only tasks. Done 2026-09-19: `reads.py` (read workers shared with the old loop), `graph.py`, `planner.py` (forced `plan_frontier`, validation, dedup, critic, hard and soft repairs), `executor.py` (`run_graph_agent`), `offline.py`, `[limits]` in the config, `harness/audits.py` with the six 7.4 checks in the audit set, `--subject graph`, run records at schema 2.0. The reschedule always runs without write authority in this phase, and `--subject graph --allow-draft-writes` is a configuration error | Offline tests 1–7, 10, 11 and 13 pass (test 12 needs the judge and moves to phase 7). `graph` verdicts equal `deterministic` verdicts on the six read-only tasks, on both tenants, in 3 runs per tenant. Planner validity, repair counts and cost per task recorded next to `llm`. Met 2026-09-19 at `6f4e740`: `graph` 36/36 pass and `deterministic` 36/36 pass (3 runs of the six read-only tasks per tenant); planner replies well-formed in 122 of 124 rounds; 13 hard and 6 soft repairs over 36 runs (at most 2 and 1 in a run); $0.21 for the 36 graph runs. The per-task table is in the README. The first exit round found two defects, fixed before the counted runs: an OpenRouter `finish_reason: error` was not retried, and rejecting `answer` dependencies cost refusals hard repairs |
| 5 | Concurrency: locks on the MCP request id and the recorders, per-call start/end sequences and node ids, pool shutdown before restore; `max_workers` above 1. Done 2026-09-19: `attribution.py` (the node id of the current call), locks in `mcp_client.py`, `harness/recorder.py` and `offline.py`; call log entries carry `node`, `start_sequence`, `end_sequence`, `started_ns` and `finished_ns`. The executor runs up to `max_workers` reads at once (1–8; default 4); each read collects evidence privately and the evidence is merged in node-id order only when nothing is running. The reschedule and `answer` run alone on the main thread after all ready reads finish; `replan = "node"` stays one node at a time. On a fatal error running reads are drained and recorded before `run_failed`. Two identical additions in one patch are now a duplicate. `harness/concurrency_check.py` checks attribution and overlap in saved records; it is not an audit | Same verdicts as phase 4 with `max_workers = 4`; journal shows overlapping reads with correct node attribution. Met 2026-09-19: `graph` 36/36 pass at `max_workers = 4` (3 runs of the six read-only tasks per tenant), `deterministic` 12/12 the same day; `concurrency_check` passes all 36 graph records, 30 of them with overlapping reads (up to 4 at once; `refuse_outside_seat` makes one read). Cost $0.24 for the 36 runs; median task time 75 s against 79 s at `max_workers = 1`, because the model, not MCP, takes most of the time |
| 6 | The write: write authority, pin, receipt, exclusive rule. Done 2026-09-19: the runner grants write authority per task (flag, `writes: true`, target selected, fixture ready) and passes it with a receipt writer to `run_graph_agent`; the executor writes `<stem>.action.json` after the once-per-run check and before `action_started` and the update, and a receipt failure fails the run with no update. `action_finished` records a `status` (`unknown` for an update whose outcome could not be confirmed). The planner prompt states the effective authority. Restore also accepts dates from an update that was interrupted before it returned. `single_subject_write` fails a receipt with no matching completed action. `--subject graph --allow-draft-writes` is no longer a configuration error. The executor does not catch interrupts (Ctrl-C): the receipt file and the restore are the record of an interrupted write | Tests 3, 8 and 9 pass. `reschedule_own_draft` passes and restores on Suryodaya with `graph`; `writes_in_scope`, `write_after_target_read` and `single_subject_write` pass. Met 2026-09-19: offline tests 3, 8 and 9 pass (`Spec: AI`); two `reschedule_own_draft` runs on Suryodaya with `graph` both `applied`, passed every verifier and audit, and restored; each wrote its receipt before its single update. A read-only six-task `graph` round on Suryodaya after the prompt change passed 6/6 |
| 7 | Spans file and judge sidecar, `--judge`. Code done 2026-09-19: `agentswitch/telemetry.py` builds `<stem>.spans.json` after `score.json` from the saved run record (journal, ledger and call log; the journal alone has no MCP calls or tokens), for every subject; ledger entries now carry `started_ns`/`finished_ns`; a spans write failure is exit 4. `agentswitch/judge.py` scores prose with one forced `score_answer` call on its own budget (`budgets.judge_usd`, `[evals]` in the config), computes `resolved`/`unresolved` locally, and records `judge_failed` for a budget refusal, provider error, missing key or unparseable reply; empty prose is `unresolved` (`no_prose`) without a call. The judge sees the request, expectation, outcome, claims and prose, never verifier results. A judge failure, including a failed judge file write, changes neither verdict nor exit code | `deterministic` verdicts unchanged; offline test 12 passes: a forced `judge_failed` changes neither verdict nor exit code. Offline part met 2026-09-19: test 12 passes (`Spec: AI`) through the CLI with the same verdict, verifiers and exit code with and without `--judge`. Live part met 2026-09-19 with phase 8's runs: `deterministic` 12/12 pass with `--judge` (all `unresolved (no_prose)`, no model call) and `graph --judge` 12/12 pass; the judge scored every graph answer through the forced `score_answer` call, with no `judge_failed` |
| 8 | Switch: `graph` becomes the LLM subject when the switch criteria in section 14 (answer 2) are met; update README and `CLAUDE.md`. The old loop stays as a frozen baseline (answer 3). Done 2026-09-19: `python -m agentswitch.agent` runs the graph agent, read-only; `--loop` runs the old loop, marked frozen; `--allow-draft-writes` needs `--loop`, and the graph agent's write runs only in the harness. Harness subjects are unchanged (`deterministic` default, `graph`, `llm`) | Team sign-off. Criteria met 2026-09-19 (README, switch evidence): every counted `graph` run passed the six read-only tasks on both tenants, as `deterministic` did the same day (90 runs across phases 4–8); two Suryodaya write runs passed and restored, and Keystone has no owned draft; late-order median cost is 0.22–0.27 of the old loop on Suryodaya and 0.47–1.07 on Keystone, and no run came near the budget. The team dropped the two-day condition. Sign-off: pending |
...
| Races in the MCP client or recorders | Phase 5 locks; the graph, journal and evidence store change only on the main thread |
...
2. **Switch criteria.** `graph` replaces `llm` as the LLM subject when all of these hold:
   - Its verdicts equal `deterministic`'s on the six read-only tasks on both tenants, in 3 runs per
     tenant. The team dropped the original "spread over at least 2 days" condition on 2026-09-19:
     the phase 4–6 runs (over 120 `graph` runs in one day, all passing after the fixed phase 4
     defect) were judged enough evidence of stability within a day. Runs from earlier phases count;
     since the phase 5 exit runs, the only change that reaches a read-only task is the sentence in
     the planner prompt that states the write authority (phase 6).
